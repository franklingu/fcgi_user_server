...
const char *check_user(const char *query_part)
{
    ...
    split_str(query_part_str, '&', records);
    ...
    if (records.size() >= 2)
    {
        ...
        {
            vector<string> items;
            split_str(records[i], '=', items);
            if (items.size() >= 2 && items[0] == "username")
            {
                username = items[1];
            }
            else if (items.size() >= 2 && items[0] == "pswd")
            {
                password = items[1];
            }
        }
...
const char *update_nick(const char *query_part)
{
    ...
    if (records.size() >= 2)
    {
        for (int i = 0; i < records.size(); i++)
        {
            ...
            else if (items.size() >= 2 && items[0] == "nickname")
            {
                nickname = items[1];
            }
        }
